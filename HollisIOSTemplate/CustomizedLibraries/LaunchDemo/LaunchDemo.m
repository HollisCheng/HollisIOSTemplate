...
#define Second_Image_Stay_Before_Fade_Out        1.0f//3.0f
#define Second_Image_Fade_Out_Animation_Duration 2.0f//3.0f
...
//		self.iconImage.frame = self.iconFrame;
        self.iconImage = [[UIImageView alloc] initWithFrame:SCREEN_BOUNDS];
	...
	dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(Second_Image_Stay_Before_Fade_Out * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
- (void)dismissAll:(JRDisApperaStyle)style {
	...
	if (style == JRDisApperaStyleOne) {
		...
		_bgImage.alpha = 0.0f;
		[UIView animateWithDuration:Second_Image_Fade_Out_Animation_Duration
							  delay:0.0f
...
- (void)showHomePage {
    DLog(@"Go to first page after splash screen");
    if(isUserLogin()){
        NSDictionary* notificationUserInfo = @{@"isAllowShowCoachMarks": [NSNumber numberWithBool:YES]};
        NWD_POST_NOTIF(kWelcomeToHomeNotification, nil, notificationUserInfo);
    }else{
        NWD_POST_NOTIF(kHomeToWelcomeNotification, nil, nil);
    }
}
