...
- (void)viewDidLoad {
    ...
    /**** edit ****/
    if (self.isFromUpdatesSection) {
        // UPDATES Tab
        _closeButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _closeButton.frame = CGRectMake((self.view.bounds.size.width-50)/2, self.view.bounds.size.height-50, 50, 50);
        [_closeButton setImage:[UIImage imageNamed:@"popup_close"] forState:UIControlStateNormal];
        [_closeButton addTarget:self action:@selector(closePhotoBrowser) forControlEvents:UIControlEventTouchUpInside];
        [self.view addSubview:_closeButton];
        
        CGFloat width = 50;
        CGFloat height = 50;
        _previousImgButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _previousImgButton.frame = CGRectMake(0, self.view.bounds.size.height-height, width, height);
        [_previousImgButton setImage:[UIImage imageNamed:@"popup_arrow_l"] forState:UIControlStateNormal];
        [_previousImgButton addTarget:self action:@selector(gotoPreviousPage) forControlEvents:UIControlEventTouchUpInside];
        [self.view addSubview:_previousImgButton];
        
        _nextImgButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _nextImgButton.frame = CGRectMake(self.view.bounds.size.width-width, self.view.bounds.size.height-height, width, height);
        [_nextImgButton setImage:[UIImage imageNamed:@"popup_arrow_r"] forState:UIControlStateNormal];
        [_nextImgButton addTarget:self action:@selector(gotoNextPage) forControlEvents:UIControlEventTouchUpInside];
        [self.view addSubview:_nextImgButton];
    }else{
        CGFloat width = 38;
        CGFloat height = 90;
        _previousImgButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _previousImgButton.frame = CGRectMake(0, (self.view.bounds.size.height-height-64)/2, width, height);
        [_previousImgButton setImage:[UIImage imageNamed:@"btn_photo_arrow_left"] forState:UIControlStateNormal];
        [_previousImgButton addTarget:self action:@selector(gotoPreviousPage) forControlEvents:UIControlEventTouchUpInside];
        [self.view addSubview:_previousImgButton];
        
        _nextImgButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _nextImgButton.frame = CGRectMake(self.view.bounds.size.width-width, (self.view.bounds.size.height-height-64)/2, width, height);
        [_nextImgButton setImage:[UIImage imageNamed:@"btn_photo_arrow_right"] forState:UIControlStateNormal];
        [_nextImgButton addTarget:self action:@selector(gotoNextPage) forControlEvents:UIControlEventTouchUpInside];
        [self.view addSubview:_nextImgButton];
    }
    /*************/
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (!_leaveStatusBarAlone && UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) {
        ...
        [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleDefault animated:animated]; //edit
    }
...
- (void)setNavBarAppearance:(BOOL)animated {
    ...
    navBar.tintColor = [UIColor blackColor]; //edit
    navBar.barTintColor = nil;
    navBar.shadowImage = nil;
    navBar.translucent = NO; //edit
    navBar.barStyle = UIBarStyleDefault; //edit
...
//            self.title = NSLocalizedString(@"Select Photos", nil); //edit
        } else {
            NSString *photosText;
            if (numberOfPhotos == 1) {
...
//            self.title = [NSString stringWithFormat:@"%lu %@", (unsigned long)numberOfPhotos, photosText]; //edit
        }
    } else if (numberOfPhotos > 1) {
        if ([_delegate respondsToSelector:@selector(photoBrowser:titleForPhotoAtIndex:)]) {
//            self.title = [_delegate photoBrowser:self titleForPhotoAtIndex:_currentPageIndex]; //edit
        } else {
//            self.title = [NSString stringWithFormat:@"%lu %@ %lu", (unsigned long)(_currentPageIndex+1), NSLocalizedString(@"of", @"Used in the context: 'Showing 1 of 3 items'"), (unsigned long)numberOfPhotos]; //edit
        }
	} else {
//		self.title = nil; //edit
	}
    ...
    /**** edit ****/
    _previousImgButton.hidden = !(_currentPageIndex > 0);
    _nextImgButton.hidden = !(_currentPageIndex < numberOfPhotos - 1);
    /*************/
